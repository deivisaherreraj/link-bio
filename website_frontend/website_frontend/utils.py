...
def next_date(dates: dict, timezone: str) -> str:
    ...
    for weekday in range(7):
        ...
        time_utc = datetime.strptime(dates[current_weekday], "%H:%M").replace(tzinfo=pytz.UTC).timetz()
        ...
        if current_time < next_time or weekday > 0:
            ...
            return local_date.strftime(f"{day}, %d de {MONTHS[local_date.month]} a las %H:%M | Zona horaria: {zones[len(zones) - 1]}")
